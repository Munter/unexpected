...
module.exports = function (expect) {
    ...
    function inspectBufferLikeObject(output, buffer, digitWidth, maxLength) {
        ...
        if (buffer.length > maxLength) {
            ...
            getHexDumpLinesForBufferLikeObject(buffer, 0, digitWidth).forEach(function (line, i) {
                if (i > 0) {
                    output.nl();
                }
                output.text(line);
    ...
    if (typeof Uint16Array !== 'undefined') {
        expect.addType({
            ...
            diff: function (actual, expected, output, diff, inspect) {
                return diff(getHexDumpLinesForBufferLikeObject(actual, 16, 4).join('\n'),
                            getHexDumpLinesForBufferLikeObject(expected, 16, 4).join('\n'));
            }
        });
    }
        ...
        diff: function (actual, expected, output, diff, inspect) {
            ...
            diffLines.forEach(function (part, index) {
                ...
                if (part.replaced) {
                    ...
                    stringDiff.diffWordsWithSpace(oldValue, newValue).forEach(function (part) {
                        if (part.added) {
    ...
    expect.addType({
        ...
        equal: function (a, b, equal) {
            ...
            try {
                expect(a.value, 'to ' + (b.exhaustively ? 'exhaustively ' : '') + 'satisfy' + (b.assertion ? ' assertion' : ''), b.value);
            } catch (e) {
                return false;
            }
            return true;
        ...
        inspect: function (obj, depth, output, inspect) {
            return output.append(inspect(obj.value));
        },
        diff: function (actual, expected, output, diff, inspect, equal) {
            function rewrapArray(obj, parent) {
                return obj.map(function (item) {
                    return {
                        ...
                        exhaustively: parent.exhaustively
                    };
                });
            }

            function rewrapObject(obj, parent) {
                var rewrappedObj = {};
            ...
            if (expected.assertion && typeof expected.value === 'string') {
                throw new Error('WHOOPS2');
            } else if (typeof expected.value === 'function') {
                try {
                    var args = [actual.value];
                    if (actual.additionalArgs) {
                        Array.prototype.push.apply(args, actual.additionalArgs);
                    }
                    expected.value.apply(expected, [actual.value].concat());
                } catch (e) {
                    return {
                        diff: e.output
                    };
            ...
            } else if (isRegExp(expected.value)) {
                try {
                    expect(actual.value, 'to match', expected.value);
                } catch (e) {
                    return {
                        diff: e.output
                    };
                ...
                var type = expect.findTypeOf(actual.value, expected.value);
                if (type === 'array') {
                    return diff(rewrapArray(actual.value, actual), rewrapArray(expected.value, expected));
                } else if (type === 'object') {
                    return diff(rewrapObject(actual.value, actual), rewrapObject(expected.value, expected));
                } else {
                    return diff(actual.value, expected.value);
                }
            }
        }
    });
