...
(function () {
    ...
    expect.addAssertion('[not] to equal', function (expect, subject, value) {
        try {
            ...
            if (!this.flags.not) {
                var comparison = expect.diff(value, subject, function (expected, actual) {
                    if (expect.equal(expected, actual)) {
                        return {
                            conflict: false,
                            text: expect.inspect(expected),
                            annotation: ''
                        };
                    } else if (typeof expected === 'undefined') {
                        return {
                            conflict: true,
                            text: expect.inspect(actual),
                        ...
                        return {
                            conflict: true,
                            text: expect.inspect(actual),
                            annotation: 'should be ' + expect.inspect(expected)
                        };
                    }
                });
                var diffString = comparison.annotation ?
                    format('error', comparison.text) + format('comment', ' // ' + comparison.annotation) :
                    comparison.text;
                e.message += '\n\n' + format('blue', 'Diff:') + '\n\n' + format('reset', diffString) + '\n';
