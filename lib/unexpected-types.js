...
(function () {
    ...
    expect.addType({
        ...
        diff: function (a, b, diff) {
            var result = {
                highlightError: false,
                annotation: ''
            };
            var keyIndex = reduce([].concat(getKeys(a), getKeys(b)), function (result, key) {
                result[key] = key;
                return result;
            }, {});
            var keys = getKeys(keyIndex);

            var diffedItems = map(keys, function (key) {
                var comparison = diff(a[key], b[key]);
                comparison.key = key;
                return comparison;
            });

            result.conflict = some(diffedItems, function (comparison) {
                return comparison.conflict;
            });

            var multipleLines = result.conflict || 50 < reduce(diffedItems, function (result, v) {
                return result + v.text.length + v.annotation.length;
            }, 0);

            var itemStrings = map(diffedItems, function (comparison, index) {
                var parts = [comparison.key + ': '];

                var text = comparison.text;

                if (multipleLines) {
                    text = text.replace(/\n/g, '\n  ');
                }

                if (comparison.conflict && comparison.annotation) {
                    parts.push(format('error', text));
                } else {
                    parts.push(text);
                }

                var last = index === diffedItems.length - 1;
                if (!last) {
                    parts.push(',');
                }

                if (comparison.annotation) {
                    parts.push(format('comment', (last ? ' ' : '') + ' // ' + comparison.annotation));
                }
    ...
    expect.addType({
        ...
        diff: function (a, b, diff) {
            ...
            var diffedItems = map(a, function (v, index) {
                return diff(v, b[index]);
            });

            for (var i = a.length; i < b.length; i += 1) {
                diffedItems.push(diff(undefined, b[i]));
            }
