...
(function () {
    ...
    expect.addType({
        ...
        diff: function (actual, expected, diff) {
            var result = {
            ...
            var diffedItems = map(keys, function (key) {
                var comparison = diff(actual[key], expected[key]);
                comparison.key = key;
                return comparison;
            });
    ...
    expect.addType({
        ...
        diff: function (actual, expected, diff) {
            ...
            var diffedItems = map(actual, function (v, index) {
                return diff(v, expected[index]);
            });

            for (var i = actual.length; i < expected.length; i += 1) {
                diffedItems.push(diff(undefined, expected[i]));
            }
