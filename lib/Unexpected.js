...
function createFn(expect, expectations) {
    function expectFn(subject) {
    ...
    expectFn._expectations = expectations;
    expectFn.and = function () {
        var copiedExpectations = expectations.slice();
        copiedExpectations.push(arguments);
        return createFn(expect, copiedExpectations);
    };
    return expectFn;
}

Unexpected.prototype.fn = function () { // ...
    return createFn(this.expect, [arguments]);
};
...
function installExpectMethods(unexpected, expectFunction) {
    ...
    expect.fn = unexpected.fn.bind(unexpected);
...
Unexpected.prototype.setErrorMessage = function (err, assertion) {
    ...
    var matchSatisfyAssertionMessage = message.toString().match(/to (exhaustively )?satisfy( assertion)?/);
    if (matchSatisfyAssertionMessage) {
        actual = {_satisfy: true, actual: true, value: assertion.subject};
        expected = {_satisfy: true, expected: true, exhaustively: !!matchSatisfyAssertionMessage[1], assertion: !!matchSatisfyAssertionMessage[2], value: assertion.args[0], additionalArgs: assertion.args.slice(1)};
    }
            ...
            try {
                ...
                if (e._isUnexpected) {
                    ...
                    if (nestingLevel === 0) {
                        ...
                        that.setErrorMessage(wrappedError, assertion);
                        throw wrappedError;
                    }
                }
                throw e;
...
var expandPattern = (function () {
    ...
    function createPermutations(tokens, index) {
        ...
        if (isFlag(token)) {
            ...
            return alternations.reduce(function (result, alternation) {
